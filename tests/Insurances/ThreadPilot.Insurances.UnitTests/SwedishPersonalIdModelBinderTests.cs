...
public class SwedishPersonalIdModelBinderTests
{
    private static DefaultModelBindingContext CreateBindingContext(string modelName, string? value)
    {
        ...
        {
            ModelMetadata = modelMetadataProvider.GetMetadataForType(typeof(SwedishPersonalId)),
            ModelName = modelName,
    ...
    [Fact]
    public async Task BindModelAsync_ShouldBind_ForValidPersonalId()
    {
        var binder = new SwedishPersonalIdModelBinder();
        var context = CreateBindingContext("personalId", "19640823-3234");

        await binder.BindModelAsync(context);

        context.Result.IsModelSet.ShouldBeTrue();
        context.Result.Model.ShouldBeOfType<SwedishPersonalId>();
        var obj = (SwedishPersonalId)context.Result.Model!;
        obj.Value.ShouldBe("19640823-3234");
    }

    [Fact]
    public async Task BindModelAsync_ShouldFail_AndAddError_ForInvalidPersonalId()
    {
        var binder = new SwedishPersonalIdModelBinder();
        var context = CreateBindingContext("personalId", "invalid");

        await binder.BindModelAsync(context);

        context.Result.IsModelSet.ShouldBeFalse();
        context.ModelState.ContainsKey("personalId").ShouldBeTrue();
    }

    [Fact]
    public void BinderProvider_ShouldReturnBinder_ForSwedishPersonalId()
    {
        var provider = new SwedishPersonalIdModelBinderProvider();
        var providerContext = new TestModelBinderProviderContext(typeof(SwedishPersonalId));
